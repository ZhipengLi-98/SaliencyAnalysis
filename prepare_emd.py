...
def run_con(user, condition, images):
    ...
    emd_g_a = []
    ys = []
    gaze_path = "./formal/imgs/{}/{}/{}_gaze.mp4".format(user, condition, condition)
    aug_path = "./formal/imgs/{}/{}/{}_ani.mp4".format(user, condition, condition)
    sal_path = "./formal/saliency/{}/{}_all.mp4".format(user, condition)
    for imgs in tqdm(images):
        ...
        sal_num = int(imgs.split(".")[0].split("frame")[1])
        test = sal_path + "/%04d.png" % (sal_num + 1)
        sal_img = cv2.imread(test, cv2.IMREAD_GRAYSCALE).astype(dtype=np.float32)
...
if __name__ == "__main__":
    for user in os.listdir("./formal/imgs"):
        user = "test"
        for condition in os.listdir(os.path.join("./formal/imgs", user)):
            print(user, condition)
            images = os.listdir(os.path.join("./formal/imgs", user, condition, condition + "_all.mp4"))
