        ...
        for imgs in tqdm(images):
            ...
            if len(contours) == 0:
                ...
                if len(innerpoints) > 0:
                    ...
                    temp_aug.append([int(np.mean(innerpoints[:, 0])), int(np.mean(innerpoints[:, 1]))])
            ...
            if len(temp_aug) > 30:
                ...
                temp_img = np.zeros((img.shape[0], img.shape[1], 1), np.float32)
                temp = np.array(temp, np.int32)
                temp = temp.reshape((-1, 1, 2))
                cv2.polylines(temp_img, [temp], isClosed=False, color=(255, 255, 255), thickness=4)
